...
- **Schema-validated specs.** Feature specifications use YAML validated by a JSON schema (`skills/needs-features/schemas/feature-spec.schema.json`). When modifying the spec format, update the schema, the validation script (`scripts/validate-specs.py`), and all example YAML files. Bump the schema's `"version"` field using SemVer (MAJOR for breaking changes, MINOR for additions, PATCH for fixes).
