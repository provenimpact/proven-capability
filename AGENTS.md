...
- **Schema-validated specs.** Feature specifications use YAML validated by a JSON schema (`skills/needs-features/schemas/feature-spec.schema.json`). When modifying the spec format, update the schema, the validation script (`skills/needs-features/scripts/validate-specs.py`), and all example YAML files. Bump the schema's `"version"` field using SemVer (MAJOR for breaking changes, MINOR for additions, PATCH for fixes).
- **Version tracking.** `spec.yaml` uses SemVer. Design tracks `:source-spec-version:` and tasks track `source_design_version` and `source_spec_version`. When modifying templates, follow this convention.
...
- **Schema-validated project artifacts.** Constraints (`docs/constraints.yaml`) and ADRs (`docs/adrs/*.yaml`) use YAML validated by JSON schemas, just like feature specs. When modifying these formats, update the corresponding schema, validation script, and example files.
- **Constraint enforcement.** Every capability checks relevant constraints from `docs/constraints.yaml` during its Evaluate phase. Constraint violations block transitions unless the user explicitly updates the constraint.
