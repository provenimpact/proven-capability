...
**Pass 2:** For each confirmed feature, invoke `needs-features` with the final slug to create `spec.yaml` with proper feature-scoped IDs. Run `python scripts/validate-specs.py` on each spec. Remove temporary `_drafts` directory.
...
3. **needs-tasks** creates `docs/features/product-browsing/tasks.yaml` v1.0.0 (3 phases, 8 tasks)
4. **needs-implementation** implements code phase by phase
...
3. **needs-tasks** creates `docs/features/shopping-cart/tasks.yaml` v1.0.0 (3 phases, 9 tasks)
4. **needs-implementation** implements code phase by phase
...
3. **needs-tasks** creates `docs/features/checkout/tasks.yaml` v1.0.0 (3 phases, 7 tasks)
4. **needs-implementation** implements code phase by phase
5. **Divergence check:** needs-implementation reports no divergences
...
2. Bump `docs/constraints.yaml` version (MINOR). Run `python scripts/validate-constraints.py`
