...
**Pass 2:** For each confirmed feature, invoke `needs-features` with the final slug to create `spec.yaml` with proper feature-scoped IDs. Run `node scripts/validate-specs.js` on each spec. Remove temporary `_drafts` directory.
...
2. Bump `docs/constraints.yaml` version (MINOR). Run `node scripts/validate-constraints.js`
