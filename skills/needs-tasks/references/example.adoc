...
* `docs/features/shopping-cart/spec.yaml` at version `1.0.0` with stories US-001 (Add to Cart) and US-002 (View Cart), requirements CART-001 through CART-007
* `docs/features/shopping-cart/design.adoc` at version `1.0.0` with system design (frontend, backend service layer, state management) and requirement resolution
...
Description:: Route handler that creates cart with session_id if none exists, delegates to CartService.addToCart. Returns updated cart JSON with confirmation message.
...
| US-001: Add to Cart
| CART-001, CART-002, CART-003
| TASK-001, TASK-003, TASK-006, TASK-007, TASK-008

| US-002: View Cart
| CART-004, CART-005, CART-006, CART-007
