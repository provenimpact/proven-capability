...
* `docs/features/shopping-cart/tasks.yaml` at version `1.0.0` with 3 phases, 9 tasks
...
* No tasks are done yet -- starting from Phase 1
...
No tasks are done. Start from Phase 1.

== Step 3: Build Phase Todo List (Phase 1: Foundation)

Parse Phase 1 from the feature's tasks.yaml. It contains 1 task:
...
Phase 1 targets requirements: CART-001, CART-004 (from TASK-001's `Requirements::` field).
...
Phase 2 targets requirements: CART-001, CART-002, CART-003, CART-006, CART-007.

Loop back to Step 3 for Phase 2.

== Updated tasks.yaml After Phase 1
  ...
  - name: Foundation
    ...
    tasks:
      - id: TASK-001
        ...
        done: true    # <-- marked done after Phase 1
